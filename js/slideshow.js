...
$msRoot.Slideshow = (function (settings) {
  ...
  Slideshow.prototype.init = function (files, slide) {
    ...
    for (var i = 0; i < files.length; i++) {
      if (this.validExtension(files[i].filename)) {
        ...
        // if invalid file extension  in list
        // the invalid file is before the chosen slide
        // adjust the chosen slide number down one
        slide = slide - 1;
      }
    }
  ...
  Slideshow.prototype.createForm = function (slide) {
    ...
    this.divWrapper.onclick = function (e) {
      e.stopPropagation();
      return false;
    };
    ...
    if (this.settings.draggable && this.settings.container !== document.body) {
      ...
      divTitleBar.innerHTML = this.settings.title;
    ...
    this.resizeWindowListener = function (e) {
      this.resizeWindow();
    }.bind(this);
    ...
    // keydown needed for arrow keys
    document.body.addEventListener("keydown", this.keypressListener, false);
    ...
    // 2019-03-14 add swipe support
    $ms.detectSwipe(this.divMiddle.id, function (id, direction) {
      switch (direction) {
        case "r":
          // previous slide
          this.loadslide(this.slide - 1, "swipe-right");
          break;
        case "l":
          // next slide
          this.loadslide(this.slide + 1, "swipe-left");
          break;
        case "u", "d":
        default:
      }
    ...
    for (var i = 0; i < this.count; i++) {
      ...
      // USED??
      this.divMiddle.onmouseenter = function (e) {
        e.stopPropagation();
        this.showNavButtons("show");
      }.bind(this);
      this.divMiddle.onmouseleave = function (e) {
        e.stopPropagation();
        this.showNavButtons("hide");
      }.bind(this);
      var _this = this;
      !function (div, i) {
        ...
        if (typeof _this.settings.cbContextMenu !== "undefined") {
          div.addEventListener('contextmenu', function (e) {
            _this.settings.cbContextMenu(div, i);
          }.bind(_this), false);
        }
      }(this.divLarge[i], i);
    }
  ...
  Slideshow.prototype.buttons = function (slide) {
    ...
    if (init) {
      this.divButtons.appendChild(divLeft.outer);
    }
    ;
    ...
    if (init) {
      this.divButtons.appendChild(divRight.outer);
    }
    ;
  ...
  Slideshow.prototype.fitButtons = function () {
    ...
    if (numberOfButtonsToHide > 0) {
      ...
      if ($ms.$(this.id + "a-first"))
        $ms.$(this.id + "a-first").style.display = "none";
      if ($ms.$(this.id + "a-playpause"))
        $ms.$(this.id + "a-playpause").style.display = "none";
      if ($ms.$(this.id + "a-last"))
        $ms.$(this.id + "a-last").style.display = "none";
    } else {
      if ($ms.$(this.id + "a-first"))
        $ms.$(this.id + "a-first").style.display = "inline-block";
      if ($ms.$(this.id + "a-playpause"))
        $ms.$(this.id + "a-playpause").style.display = "inline-block";
      if ($ms.$(this.id + "a-last"))
        $ms.$(this.id + "a-last").style.display = "inline-block";
    }
  };
  ...
  Slideshow.prototype.firstLink = function () {
    ...
    if (this.slide === 0) {
      ...
      a.onclick = function (e) {
        e.stopPropagation();
        this.loadslide(0, "button-first-click");
      }.bind(this);
  ...
  Slideshow.prototype.previousLink = function () {
    ...
    if (this.slide === 0 && !this.settings.slideshowWrap) {
      ...
      a.onclick = function (e) {
        e.stopPropagation();
        this.loadslide(this.slide - 1, "button-previous-click");
      }.bind(this);
  ...
  Slideshow.prototype.nextLink = function () {
    ...
    if (this.slide >= this.count - 1 && !this.settings.slideshowWrap) {
      ...
      a.onclick = function (e) {
        e.stopPropagation();
        this.loadslide(this.slide + 1, "button-next-click");
      }.bind(this);
  ...
  Slideshow.prototype.lastLink = function () {
    ...
    if (this.slide >= this.count - 1) {
      ...
      a.onclick = function (e) {
        e.stopPropagation();
        this.loadslide(this.count - 1, "button-last-click");
      }.bind(this);
  ...
  Slideshow.prototype.magnifierLink = function () {
    ...
    if (!img) {
      ...
      if (this.settings.zoomMode === "zoom") {
        ...
        a.onclick = function (e) {
          e.stopPropagation();
          this.zoomImageStart(true);
        }.bind(this);
      } else {
        // magnifier mode
        a.onclick = function (e) {
          e.stopPropagation();
          this.startMagnifier(e);
        }.bind(this);
      }
      img.src = $ms.STATIC_IMG_COMMON + '/blank.gif';
      img.alt = 'Zoom In';
  ...
  Slideshow.prototype.wheel = function (event) {
    ...
    if (delta) {
      ...
      if ($ms.$(this.id + "magnifier")) {
        ...
        this.zoomImageStart(delta === 1, {x: event.pageX, y: event.pageY});
      }
    }
  ...
  Slideshow.prototype.keypress = function (e) {
    ...
    switch (e.keyCode) {
      ...
      case 27:
        ...
        if (this.closeMagnifier())
          return;
        // escape otherwise closes slideshow if allowed
        if (this.settings.escapeKeyCloses)
          this.close(e);
        break;
      case 37:    // left
        e.preventDefault();
  ...
  Slideshow.prototype.zoomImage = function (e) {
    ...
    //******** 2022 ********
    var newImgWidth = parseInt(this.largeSize[this.slide].displayWidth) * (this.zoom);
    var newdivLargeWidth = Math.min(parseInt(this.divWrapper.style.width), newImgWidth);
    var imgOverflow = newImgWidth - newdivLargeWidth;
    var divLargeGrowth = newdivLargeWidth - parseInt(this.divLarge[this.slide].style.width);
    var imgGrowth = newImgWidth - parseInt(this.img[this.slide].style.width);
    this.divLarge[this.slide].style.width = newdivLargeWidth + "px";
    var margin = (parseFloat(this.divWrapper.style.width) - parseFloat(this.divLarge[this.slide].style.width)) / 2;
    this.divLarge[this.slide].style.left = margin + "px";
    xStart += divLargeGrowth / 2;
    //**********************

    this.img[this.slide].style.height = parseInt(this.divLarge[this.slide].style.height) * (this.zoom) + "px";
    this.img[this.slide].style.width = newImgWidth + "px";   // 2022 => parseInt(this.divLarge[this.slide].style.width) * (this.zoom) + "px";
  ...
  Slideshow.prototype.resetZoom = function (divLarge, img) {
    ...
    // 2022
    // reset divLarge to original size
    for (var slide = 0; slide < this.divLarge.length; slide++) {
      if (this.divLarge[slide].id === divLarge.id) {
        this.divLarge[slide].style.width = parseInt(this.largeSize[slide].displayWidth) + "px";
        var margin = (parseFloat(this.divWrapper.style.width) - parseFloat(this.divLarge[slide].style.width)) / 2;
        this.divLarge[slide].style.left = margin + "px";
        break;
      }
    }
  ...
  Slideshow.prototype.validExtension = function (file) {
    var validExt = [".jpg", ".jpeg", ".bmp", ".gif", ".png"];
    if (file.length > 0) {
      var valid = false;
      for (var j = 0; j < validExt.length; j++) {
        if (file.substr(file.length - validExt[j].length, validExt[j].length).toLowerCase() === validExt[j].toLowerCase()) {
          valid = true;
          break;
        }
      }
  ...
  Magnifier.prototype.initmagnifier = function () {
    ...
    if (this.settings.divExternalMagnifier) {
      ...
      if (this.settings.magnifierStyles) {
        ...
        // 2022 set default position of magnifier so can be seen
        this.settings.divExternalMagnifier.style.top = "10px";
        this.settings.divExternalMagnifier.style.left = "625px";
        this.settings.divExternalMagnifier.style.width = "600px";
        this.settings.divExternalMagnifier.style.height = "600px";
        this.settings.divExternalMagnifier.style.border = "1px solid black";        
      }
  ...
  MSTouch.prototype.pinchMove = function (e) {
    ...
    if (e.touches.length === 2) {
      ...
      for (var i = 0; i < this.tpCache.length; i++) {
        if (this.tpCache[i].identifier === e.touches[0].identifier)
          point1 = i;
        if (this.tpCache[i].identifier === e.touches[1].identifier)
          point2 = i;
      }
